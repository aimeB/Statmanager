...
import {
  ChampionnatOverviewDTO,
  ChampionnatDetailWithRencontresDTO,
  ChampionnatDTO,
  ChampionnatCreationDTO,
} from './../models/championnat.model'; // par ex. si tu en as un
...
export class ChampionnatService {
  ...
  getChampionnatOverview(idChampionnat?: number): Observable<ChampionnatOverviewDTO> {
    const url = idChampionnat
      ? `${this.apiUrl}/overview/${idChampionnat}`
  ...
  getStatistiquesRencontre(idRencontre: number): Observable<StatistiquesRencontreDTO[]> {
    return this.http.get<StatistiquesRencontreDTO[]>(
      `${this.apiUrl}/rencontres/${idRencontre}/stats`,
    );
  }
